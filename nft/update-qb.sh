...
# nft
retry_on_fail "nft add chain inet fw4 qb_dstnat"
retry_on_fail "nft flush chain inet fw4 qb_dstnat"
if ! nft list chain inet fw4 dstnat | grep -q 'jump qb_dstnat' > /dev/null; then
  retry_on_fail "nft add rule inet fw4 dstnat jump qb_dstnat"
fi
retry_on_fail "nft add rule inet fw4 qb_dstnat iifname $interface $protocol dport $private_port counter dnat ip to $host:$port"
...
if [ $forward_ipv6 -eq 1 ]; then
  retry_on_fail "nft add chain inet fw4 qb_forward_wan"
  retry_on_fail "nft flush chain inet fw4 qb_forward_wan"
  if ! nft list chain inet fw4 forward_wan | grep -q 'jump qb_forward_wan' > /dev/null; then
    retry_on_fail "nft insert rule inet fw4 forward_wan jump qb_forward_wan"
  fi
  retry_on_fail "nft add rule inet fw4 qb_forward_wan iifname $interface meta nfproto ipv6 tcp dport $port counter accept"
  retry_on_fail "nft add rule inet fw4 qb_forward_wan iifname $interface meta nfproto ipv6 udp dport $port counter accept"
